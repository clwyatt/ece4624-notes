...
[Note:]{.alert} if you need to see me outside office hours, send me an email with a selection of times over the following 2-3 days and I will get back to you with a time and location if possible.
...
Example Problem: Given a low-pass filter described by the difference equation
  $$
  y[n] = ay[n-1] + by[n-2] + cy[n-3] + dx[n] + ex[n-1] + fx[n-2]
  $$
  where the coefficients $a$ through $f$ are given and whose transition band is defined from -3dB to -40dB, determine the cutoff frequency and width of the transition band in samples/sec.
...
* The first problem set covers CT/DT signals and the CT Fourier Transform. It is due Wed 9/10 [through Canvas]{.underline}.
* You should typeset your solutions, use an annotating program on the PDF, or scan work done with pencil and
paper (the free Adobe Scan app works well).
...
- A group of two or three works well. In these groups you are allowed to
